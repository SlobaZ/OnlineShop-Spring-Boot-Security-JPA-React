...
const ITEMS_API_BASE_URLS = "http://localhost:8080/api/shoppings/";
    ...
    const buyItem = (itemId, itemQuantity) => {
        return axios.get(ITEMS_API_BASE_URL + '/' + itemId + '/' + itemQuantity + '/buyItem' ,  { headers: AuthenticationHeader() } );
    }
    
    const resetItem  = (itemId) => {
        return axios.get(ITEMS_API_BASE_URL + '/' + itemId  + '/resetItem' ,  { headers: AuthenticationHeader() } );
    }
